import style from "../template8/template8.module.scss";
import React,{useRef,useState,useEffect} from 'react';
import data4 from "../../mobiledata4";
import Slider from "react-slick";
import { Popup } from "../popup/Popup";
import { Pricing,MobileData,Settings } from "../../utils/formatter";
...
export const Template8 = () => {
 ...
 const[isOpen,setIsOpen]=useState<boolean>(false);
  ...
  const arrowcolor:string='#212121';
  useEffect(()=>
  {
    if(isOpen || show)
    {
  ...
  <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 19.5L8.25 12l7.5-7.5" />
    ...
    { data4.map((item:MobileData)=>
    (
    <>
    <div className="cardmainWrapper"
    onMouseOver={(e)=>e.currentTarget.style.border=`1px solid ${item.buttonColor}`}
       onMouseOut={(e)=>e.currentTarget.style.border=`1px solid ${item.primaryColor==='#223F80'?'transparent':'#E7E7E7'}`}
    >
    ...
    <div className="cardsubContent">
     ...
     <p className={style["warningTextMobile"]} style={{color:item.primaryColor==='#FFF'?'#9C9B9E':'#9FB5D6'}}>Cancel anytime,non-refundable</p>
      { 
      item.tiers?.length>1 && 
      <Dropdown2 
    ...
    <button className='actionmbutton4' style={{color:item.primaryColor,backgroundColor:item.buttonColor}}>{item?.id===cardid && (coupon!=='' && coupon!==undefined)?`Proceed ₹${pricingObj?.price}`: `Subscribe`}</button>
    {activeIndex!==item.id?
          (<p 
    ...
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.4" stroke="#FFF" className="arrow-right"> 
    <path stroke-linecap="round" stroke-linejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5"/>
    </svg>
    </div>
    </div>
    <Modal show={show} setShow={setShow} data={data4} cardId={cardid}/>
{isOpen && <Popup coupontext={coupon} setcoupon={setCoupon} modal={isOpen} setModal={setIsOpen} setId={setActiveIndex}/>}
    </>
    );
}
